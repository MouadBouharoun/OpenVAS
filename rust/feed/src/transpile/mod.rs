...
pub enum FindParameter {
    ...
    /// Find a paramter by index
    Index(usize),
}

/// Is used within Replacer to find a specific statment to operato on.
#[derive(Clone, Debug, serde::Serialize, serde::Deserialize)]
pub enum Find {
...
impl<'a> Matcher for FunctionNameMatcher<'a> {
    fn matches(&self, s: &Statement) -> bool {
        ...
        let (named, anon) = match s {
            Statement::Include(_, _, ..) | Statement::Exit(_, _, ..) => (vec![], 1),
            Statement::Call(_, p, ..) => {
                let named = p
                    .iter()
            ...
            Statement::FunctionDeclaration(_, _, p, _, _block) => {
                let anon = {
                    ...
                    // the given indeces number when available
                    //
                    ...
                    // anon args when finding a funktion declaration as this limitation is more obvious
        ...
        for w in wanted {
            let result = match w {
                FindParameter::Name(name) => named.iter().find(|n| &n.0 == name).is_none(),
                FindParameter::Index(x) => x != &anon,
                FindParameter::NameValue(n, v) => {
                    dbg!(n, v, &named);
                    named.iter().find(|(k, ov)| k == n && ov == v).is_none()
                }
            };
            if result {
                return false;
...
impl CodeReplacer {
        ...
        match r {
            ...
            Replace::Parameter(params) => {
                ...
                match params {
                    ...
                    ParameterOperation::Remove(i) => self.remove_indexed_paramter(s, *i),
    ...
    pub fn replace(code: &str, replace: &[ReplaceCommand]) -> Result<String, Box<dyn Error>> {
        ...
        for r in replace {
            ...
            // TODO may cache and override on change
    ...
    fn push_parameter(&mut self, s: &Statement, p: &Parameter) {
        ...
        if let Some((pos, np)) = match s {
            ...
            self.replace_range(&npos, &param, &npos)
        }
    }
    ...
    fn add_parameter(&mut self, s: &Statement, i: usize, p: &Parameter) {
        ...
        match s {
                ...
                if i <= params.len() || i == 0 =>
            {
                ...
                if let Some(s) = np {
                    let position = index_exits.unwrap_or_else(|| end.position);
                    let new_position = self.range_with_offset(&position);
                    let before = &self.code[new_position.0..new_position.1];
                    self.replace_range(&new_position, &format!("{s}{before}"), &new_position);
                }
            }
            _ => {}
        }
    ...
    fn remove_indexed_paramter(&mut self, s: &Statement, i: usize) {
        match s {
            Statement::FunctionDeclaration(_, _, stmts, ..) | Statement::Call(_, stmts, ..) => {
...
impl<'a> Iterator for FeedReplacer<'a> {
    type Item = Result<Option<String>, Box<dyn Error>>;
...
mod parsing {
    ...
    fn generate_replace_commands() -> Vec<ReplaceCommand> {
            ...
            ReplaceCommand {
                ...
                with: crate::transpile::Replace::Parameter(
                    crate::transpile::ParameterOperation::Rename {
                        previous: "service-to-be".to_string(),
    ...
    fn to_toml() {
        ...
        let aha = toml::to_string_pretty(&w).unwrap();
        println!("{aha}");
        panic!("aha")
    }
}
    ...
    fn find_parameter() {
        ...
        let replaces = [
            ...
            ReplaceCommand {
                find: Find::FunctionByNameAndParameter(
                    "funker".to_string(),
                    vec![FindParameter::Index(1 as usize)],
                ),
